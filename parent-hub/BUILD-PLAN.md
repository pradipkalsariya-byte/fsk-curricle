...
- Live URL once pushed: `https://vardan-kabra.github.io/fsk-curricle/parent-hub/index.html`
...
2. **Logo in hero.** The brand SVG (`assets/brand/FSK - SVG Logo.svg`) reads
...
`FSK Parent Information Packet Sample (1) (1).html` (user's Downloads, not in repo).
...
- Headline + short warm welcome line addressing **all** FSK parents (no IB-vs-CBSE split).
...
**⚠ DO NOT RENDER** (internal/operational — keep OUT of the HTML): `info@fsk.in` must
...
- Parent address format: `p.firstname.lastname@fsk.in` (`p.`=parent · `firstname.lastname`=child's name · `fsk.in`=domain).
...
- **Stream A — Admin** (transport, food, I-Card, uniform, fees logistics):
  1. **Primary:** email `info@fsk.in` (actively encouraged).
...
- **Stream B — Academic** (learning, child's progress, classroom):
  1. **Primary:** email your child's teacher directly (`firstname.lastname@fsk.in`; also
...
| Name | Role | Email |
|---|---|---|
| Pradeep Sharma | Head of School | pradeep.sharma@fsk.in |
| Abhijit Dive | Head of Admin & CBSE Principal | abhijit.dive@fsk.in |
| Kiran Kante | Admin Operations Manager | kiran.kante@fsk.in |
| Hetal Ahivasi | PYP Coordinator | hetal.ahivasi@fsk.in |
| Anjali Bindra | TL — Early Years & Single Subjects (PYP) | anjali.bindra@fsk.in |
| Anisha Law | PYP TL — Grade 1–3 | anisha.law@fsk.in |
| Zarina Khan | PYP TL — Grade 4–5 | zarina.khan@fsk.in |
| Vichitra Parmal | MYP Coordinator | vichitra.parmal@fsk.in |
| Bhavana Goraksha | DP Coordinator | bhavana.goraksha@fsk.in |
| Farhat Khan | CBSE & IB Processes Coordinator | farhat.khan@fsk.in |
| Shailendra Law | Whole School Sports & PE TL | shailendra.law@fsk.in |

Quick reference: Admin → `info@fsk.in` · WhatsApp/calls → `9657662888` (text preferred ·
no voice notes · calls = emergencies only) · Admissions & Inquiries → `9274770453`.
Admissions referral note: *"Know someone interested in FSK? Share our Admissions &
...
Nucleus · Daily timetable → ASC Timetable · 1:1 with a teacher → school email · Admin → info@fsk.in.
...
- [ ] Internal `info@fsk.in` routing note is NOT in the HTML.
