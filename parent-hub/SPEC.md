...
- Welcome line (short, warm) — addresses all FSK parents (does **not** call out IB vs CBSE as separate streams)
...
**Source:** the user's separate Build Spec doc ("FSK Parent Communication System - Build Spec.md") — canonical for content. Content goes straight into the rendered HTML; this section captures structure only.
...
**4 pillars** (in order):
1. **Your school email ID** — parent address format `p.firstname.lastname@fsk.in`. **Prominently display** the "enter YOUR age, not your child's" warning — the single most consequential item on this page.
...
3. **How you reach us (inbound)** — two streams:
   - **Admin** (transport/food/I-Card/uniform/fees logistics): email `info@fsk.in` > WhatsApp text `9657662888` > phone `9657662888` (emergencies only)
...
**⚠ DO NOT RENDER:** the internal note about `info@fsk.in` being a Google Group routed to internal owners. Parents see only the clean address. (This note is operational; not user-facing.)
