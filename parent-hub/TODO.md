...
- 🔭 **Directors** at the top. Email **directors@fountainheadschools.org**
  ⚠️ domain is **fountainheadschools.org**, *not* `fsk.in`. ❓ Need Director name(s) + placement.
...
- 🔭 Add a card in **Essentials** for the **ASC Timetable** (`asc-timetable` → `fsk.edupage.org/timetable/`) showing an **embedded view** of the link.
...
- ✅-to-do **Email format mismatch.** Overview card (Pillar 1, ~L614) shows `p.firstname@fsk.in`;
  detail (~L643) shows `p.firstname.lastname@fsk.in`. Kiran confirmed: it should be
  **`p.firstname.lastname@fsk.in`** everywhere. Fix the card.
...
- ⏳ **Key dates** (Suparna) → parent-facing dates **extracted** from the FSK Student's Calendar → `parent-hub/key-dates.md` (review/trim; see its open questions). Presentation (static list vs live sheet-backed strip) still pending user comments. (A key-dates table was dropped earlier as "too dynamic" — team now wants it back.)
...
- **Batch build (2026-06-10)** — verified on preview, **pending commit**:
  - Quick fixes: email card → `p.firstname.lastname@fsk.in`; phone display digits-only (`9657662888` / `9274770453`, `tel:`/`wa.me` hrefs keep `+91`).
