/**
 * FSK — "My Bus" (parent-specific bus & route lookup).
 *
...
function doGet(e) {
  ...
  if (!child) return html_(pageShell_('We couldn’t find your details yet',
    ...
    '<a href="mailto:info@fsk.in">info@fsk.in</a> and we’ll sort it out.</p>'));
...
function pageShell_(heading, inner) {
  return STYLE_ + '<div class="wrap"><div class="card"><h1>' + esc_(heading) + '</h1>' +
    inner + '<p class="back"><a href="https://fsk-curricle.pages.dev/parent-hub/">← Back to the Parent Hub</a></p>' +
    '</div></div>';
}
...
function renderMyBus_(child, route) {
  ...
  var stopBlock = hasStop
    ...
    : '<p class="note2">Your child’s specific stop isn’t marked yet — find it in the route below, or write to ' +
      '<a href="mailto:info@fsk.in">info@fsk.in</a>.</p>';
    ...
    '<div class="wrap">' +
      ...
      '<div class="card">' +
        ...
        (route.stops.length
          ...
          : '<p>Route details for this bus aren’t available yet — please write to <a href="mailto:info@fsk.in">info@fsk.in</a>.</p>') +
        '<p class="hint">Pick-up times are morning (to school); drop times are afternoon (home).</p>' +
      '</div>' +
      '<p class="back"><a href="https://fsk-curricle.pages.dev/parent-hub/">← Back to the Parent Hub</a></p>' +
    '</div>';
}
