...
For real `@fsk.in` parents: deploy under an `@fsk.in` account so parents are same-domain. If you need
a single deployment to identify **both** `@fsk.in` parents and `@fountainheadschools.org` testers, tell
