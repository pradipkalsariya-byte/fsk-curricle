/* FSK Curricle — Google sign-in gate (used by the bus app only).
 *
...
(function () {
  ...
  var cfg = window.FSK_CONFIG || {};
  var TKEY = (cfg.cachePrefix || "fsk:v1:") + "idtoken";
  ...
  if (!document.getElementById("fsk-auth-style")) {
    var st = document.createElement("style");
    st.id = "fsk-auth-style";
    st.textContent = [
      ".fsk-auth{position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;padding:24px;",
      ...
      ".fsk-auth-card{background:var(--panel,#fff);border:1px solid var(--line,#dde5ec);border-radius:18px;",
      ...
      ".fsk-auth-card .kicker{display:inline-flex;align-items:center;gap:8px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;",
      ...
      ".fsk-auth-card h2{font-family:var(--serif,Georgia,serif);font-weight:600;font-size:26px;margin:16px 0 8px;color:var(--ink,#16202b)}",
      ".fsk-auth-card p{color:var(--muted,#5a6b7b);font-size:15.5px;line-height:1.5;margin:0 0 18px}",
      ".fsk-auth-btn{display:flex;justify-content:center;min-height:44px}",
      ".fsk-auth-note{font-size:13px;color:var(--muted,#5a6b7b);margin-top:18px}",
    ].join("");
  ...
  function showGate() {
    ...
    ov.className = "fsk-auth";
    ov.innerHTML =
      '<div class="fsk-auth-card">' +
        ...
        '<div class="fsk-auth-btn" id="fskGoogleBtn"></div>' +
        '<p class="fsk-auth-note"></p>' +
      "</div>";
    document.body.appendChild(ov);
    overlayEl = ov;
    var note = ov.querySelector(".fsk-auth-note");
    ...
    loadGis(function () {
      try {
        ...
        google.accounts.id.renderButton(ov.querySelector("#fskGoogleBtn"), {
