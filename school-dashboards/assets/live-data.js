...
(function () {
  ...
  var cfg = (window.FSK_CONFIG = window.FSK_CONFIG || {});
  var PREFIX = cfg.cachePrefix || "fsk:v1:";
  ...
  if (!document.getElementById("fsk-live-style")) {
    var st = document.createElement("style");
    st.id = "fsk-live-style";
    st.textContent = [
      ".fsk-live{display:flex;align-items:center;gap:9px;width:fit-content;max-width:100%;",
      ...
      ".fsk-live .dot{width:9px;height:9px;border-radius:50%;background:var(--muted,#5a6b7b);flex:none}",
      ".fsk-live.ok .dot{background:var(--brand,#1d6f5c);box-shadow:0 0 0 3px var(--brand-soft,#e4f1ec)}",
      ".fsk-live.loading .dot{background:var(--gold,#d8a531);animation:fskPulse 1s ease-in-out infinite}",
      ".fsk-live.stale .dot{background:var(--gold,#d8a531)}",
      ".fsk-live .msg{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
      ".fsk-live .retry{font-family:inherit;font-weight:700;font-size:13px;color:var(--brand-ink,#0f4a3d);",
      "background:none;border:none;cursor:pointer;padding:2px 4px;text-decoration:underline;flex:none}",
      "@keyframes fskPulse{0%,100%{opacity:1}50%{opacity:.35}}",
      ".fsk-live-error{text-align:center;color:var(--muted,#5a6b7b);padding:40px 20px;",
      "border:2px dashed var(--line,#dde5ec);border-radius:18px;background:rgba(255,255,255,.6);margin:8px 0}",
      ".fsk-live-error .big{font-size:42px;margin-bottom:10px}",
      ".fsk-live-error .retry{margin-top:14px;font-family:inherit;font-weight:700;font-size:15px;color:#fff;",
  ...
  function init(opts) {
    ...
    pill.className = "fsk-live loading";
  ...
  window.FSKLiveData = { init: init, timeAgo: timeAgo };
})();
