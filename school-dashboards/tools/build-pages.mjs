 ...
 *   - replaces the bootstrap call with FSKLiveData.init(...) wiring
 */
...
function buildMeals() {
  ...
  const init = [
    ...
    '}',
    "FSKLiveData.init({ sheet:'meals', statusMount:document.getElementById('liveStatus'), onData:applyMealData });",
    '</script>',
  ].join('\n');
...
function buildBuses() {
  ...
  const init = [
    ...
    '    onData: applyBusData,',
    '    getToken: function(){ return window.FSKAuth ? FSKAuth.getToken() : null; },',
    "    onAuthRequired: function(){ if(window.FSKAuth){ FSKAuth.clear(); FSKAuth.gate({ onAuth: function(){ location.reload(); } }); } }",
    '  });',
    '}',
    '// Gate the bus app behind Google sign-in when enabled, else load directly.',
    'if (window.FSK_CONFIG && window.FSK_CONFIG.gateBuses && window.FSKAuth) {',
    '  FSKAuth.gate({ onAuth: startBuses });',
    '} else {',
    '  startBuses();',
    '}',
    '</script>',
